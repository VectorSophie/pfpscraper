...
3. **Save:** `display_avatar` (server avatar if set, else global) as static PNG, 512px → write `<output_dir>/<YYMMDD>/<slug>.png` (one folder per day). Skip write if that file exists (no same-day churn). Update `state.json`.
